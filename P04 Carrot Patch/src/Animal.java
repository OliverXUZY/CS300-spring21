...
public class Animal implements GUIListener {
   ...
   * @param range       radius of the neighborhood range from the position of this Animal * @return
   *                    true if otherAnimal is close to the current tiger
   */
  public boolean isClose(Animal otherAnimal, int range) {
    // TODO implement this method
    if(otherAnimal.isClose(this.x, this.y, range)) {
      return true;
    }
    else {
      return false;
