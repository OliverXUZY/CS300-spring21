  ...
  public void hopTowardsCarrot() {
    ...
    if (carrot != null) {
      ...
      if (this.isClose(carrot[1], carrot[2], hopStep)) {
        ...
        this.setX(this.getX() + hopStep * (carrot[1] - this.getX()) / d);
        this.setY(this.getY() + hopStep * (carrot[2] - this.getY()) / d);
      }
    }
  }
  ...
  public boolean watchOutForWolf() {
    ...
    for(int i = 0; i < processing.objects.size(); i++) {
      if(processing.objects.get(i) instanceof Wolf) {
        if (this.isClose((Wolf)processing.objects.get(i), Rabbit.getScanRange())) {
          return true;
        }
      }
    }
